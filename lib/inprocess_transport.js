...
module.exports = function () {
	return function createInprocessTransport(api) {
		...
		self.send = function (pattern, payload) {
			var handler = api.findSingle('local-queue', pattern);
		...
		self.addHandler = function (pattern, handler) {
			return Promise.resolve(api.addSingle('local-queue', pattern, handler));
		};

		self.removeHandler = function (pattern, handler) {
			return Promise.resolve(api.removeSingle('local-queue', pattern, handler));
		};
