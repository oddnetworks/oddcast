	...
	queryHandler: function (pattern, handler) {
		this.requests.respond(pattern, function (args) {
			return new Promise(function queryHandlerWrapper(resolve, reject) {
				if (handler.length > 1) {
					handler(args, pattern, function (err, res) {
						if (err) {
							reject(err);
						} else {
							resolve(res);
					...
					var res = handler(args, pattern);
					if (res && typeof res.then === 'function') {
						res.then(resolve, reject);
					} else {
						reject(new Error('A Promise is expected from a queryHandler when ' +
	...
	commandHandler: function (pattern, handler) {
		...
		this.commands.receive(pattern, function (action) {
			...
			var promise = new Promise(function commandHandlerWrapper(resolve, reject) {
				if (handler.length > 1) {
					handler(payload, pattern, function (err, res) {
						if (err) {
							reject(err);
						} else {
							resolve(res);
					...
					var res = handler(payload, pattern);
					if (res && typeof res.then === 'function') {
						res.then(resolve, reject);
					} else {
						reject(new Error('A Promise is expected from a commandHandler when ' +
							'no callback is supplied'));
					}
				}
			});
			...
			return promise.then(
				function (res) {
					var thisAction = Action.withResult(action, res);
					self.events.broadcast(thisAction.pattern, thisAction);
					return true;
				},
				function (err) {
					var thisAction = Action.withError(action, err);
					self.events.broadcast(thisAction.pattern, thisAction);
					return false;
				}
			);
