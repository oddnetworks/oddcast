...
utils.extend(Bus.prototype, {
			...
			return new Promise(function queryHandlerWrapper(resolve, reject) {
				if (handler.length > 1) {
					...
					if (res && typeof res.then === 'function') {
						...
						reject(throw new Error('A Promise is expected from a queryHandler when ' +
	...
	commandHandler: function (pattern, handler) {
		...
		this.commands.receive(pattern, function (action) {
			...
			var promise = new Promise(function commandHandlerWrapper(resolve, reject) {
				if (handler.length > 1) {
					...
					if (res && typeof res.then === 'function') {
						...
						reject(throw new Error('A Promise is expected from a commandHandler when ' +
							'no callback is supplied'));
					}
				}
			});
			...
			return promise.then(
				function (res) {
					var thisAction = Action.create(action).withResult(res);
					self.events.broadcast(thisAction.pattern, thisAction);
					return true;
				},
				function (err) {
					var thisAction = Action.create(action).withError(err);
					self.events.broadcast(thisAction.pattern, thisAction);
					return false;
				}
			);
